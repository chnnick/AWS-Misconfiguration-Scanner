...
def check_function_code(function):
    function_name = function["FunctionName"]
    try:
        code_url = lambda_client.get_function(FunctionName=function_name)["Code"].get("Location")
        if not code_url:
            return None
...
def scan_lambda():
    ...
    for function in functions:
        ...
        for check in [check_env_vars, check_function_code]:
...
def run_scanner():
    ...
    nodes, relationships = scan_lambda()
